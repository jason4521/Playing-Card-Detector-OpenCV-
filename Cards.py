...
def preprocess_card(contour, image):
    ...
    # 求出外接矩形的宽高
    x,y,w,h = cv2.boundingRect(contour) #求外接矩形
...
def flattener(image, pts, w, h):
    ...
    if w <= 0.8*h: # If card is vertically oriented 垂直
        temp_rect[0] = tl
        temp_rect[1] = tr
        temp_rect[2] = br
        temp_rect[3] = bl

    if w >= 1.2*h: # If card is horizontally oriented 水平
        temp_rect[0] = bl
        temp_rect[1] = tl
        temp_rect[2] = tr
        temp_rect[3] = br
    ...
    if w > 0.8*h and w < 1.2*h: #If card is diamond oriented
        # If furthest left point is higher than furthest right point, 如果最左边的点高于最右边的点
        # card is tilted to the left. 左倾
        ...
        # If furthest left point is lower than furthest right point, 如果最左边的点比最右边的点低
        # card is tilted to the right 右倾
        if pts[1][0][1] > pts[3][0][1]:
...
def flattener(image, pts):
    ...
    pt2 = tuple(pts[2][0])
    pt3 = tuple(pts[3][0])
    temp_rect = np.zeros((4, 2), dtype="float32") #存放4个点坐标
    if len(pts)==4:
        dist_01 = np.sqrt(np.sum((pts[0][0] - pts[1][0])**2))
        dist_12 = np.sqrt(np.sum((pts[1][0] - pts[2][0]) ** 2))
        print(dist_01,dist_12)
        # 透视变换的输入点是顺时针（起始点是tl点），角点是逆时针（起始点是最高点），需要一一对应
        if dist_01>dist_12:
            temp_rect[0] = pts[0][0] #tl,red point
            temp_rect[1] = pts[3][0] #bl
            temp_rect[2] = pts[2][0] #br
            temp_rect[3] = pts[1][0] #tr
        if dist_01<dist_12:
            temp_rect[0] = pts[1][0] #tl,red point
            temp_rect[1] = pts[0][0] #bl
            temp_rect[2] = pts[3][0] #br
            temp_rect[3] = pts[2][0] #tr
